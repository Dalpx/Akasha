...
class _ClientesPageState extends State<ClientesPage> {
  ...
  String? _validarDocumento(String? value, String tipoDocumento) {
    ...
    } else if (tipoDocumento == '2') { // Pasaporte
      final pasaporteRegExp = RegExp(r'^P-[A-Z0-9]{9,15}$', caseSensitive: false);
  ...
  Future<void> _abrirDialogoNuevoCliente() async {
    ...
    final TextEditingController tipoDocumentoController = TextEditingController(text: '1');
    ...
    final List<Cliente> clientesExistentes = await _clienteService.obtenerClientesActivos();
          ...
          builder: (
            ...
            return AlertDialog(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      TextFormField(
                        ...
                        validator: (value) {
                          return _validarDocumento(value, tipoDocumentoSeleccionado);
                        },
                      ),
              ...
              actions: <Widget>[
                ...
                ElevatedButton(
                  onPressed: () async {
                    final String nroDocumento = nroDocumentoController.text.trim();

                    // Verificar unicidad del documento
                    bool existe = clientesExistentes.any(
                      (c) => c.nroDocumento.toLowerCase() == nroDocumento.toLowerCase(),
                    );

                    if (existe) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Ya existe un cliente con este número de documento'),
  ...
  Future<void> _abrirDialogoEditarCliente(Cliente cliente) async {
    ...
    final TextEditingController nombreController = TextEditingController(text: cliente.nombre);
    final TextEditingController apellidoController = TextEditingController(text: cliente.apellido);
    final TextEditingController tipoDocumentoController = TextEditingController(text: cliente.tipoDocumento);
    final TextEditingController nroDocumentoController = TextEditingController(text: cliente.nroDocumento);
    ...
    final TextEditingController direccionController = TextEditingController(text: cliente.direccion ?? '');

    String tipoDocumentoSeleccionado = cliente.tipoDocumento;
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (existe) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Ya existe otro cliente con este número de documento'),
                          backgroundColor: Colors.red,
                        ),
                      );
                      return;
                    ...
                    if (formKey.currentState!.validate()) {
                      ...
                      cliente.email = emailController.text.trim().isNotEmpty 
                          ? emailController.text.trim() 
                          : null;
                      cliente.direccion = direccionController.text.trim().isNotEmpty
                          ? direccionController.text.trim()
                          : null;
