...
class _ReportesPageState extends State<ReportesPage> {
  ...
  Future<_ReporteData> _cargarDatosReporte() async {
    ...
    try {
      final resultados = await Future.wait([
        ...
        widget.inventarioService.obtenerReporteValorado().catchError(
          (_) => <Map<String, dynamic>>[],
        ),
        widget.inventarioService.obtenerReporteSinStock().catchError(
          (_) => <Map<String, dynamic>>[],
        ),
        widget.inventarioService.obtenerHistorialMovimientos().catchError(
          (_) => <Map<String, dynamic>>[],
        ),
        widget.inventarioService.obtenerReporteStockPorUbicacion().catchError(
          (_) => <Map<String, dynamic>>[],
        ),
        widget.inventarioService.obtenerReporteAABC().catchError(
          (_) => <Map<String, dynamic>>[],
        ), // <--- NUEVA LLAMADA
      ]);
    ...
    double valorInventario = inventario.fold(
      0.0,
      (sum, item) => sum + (item['valor_total'] as num? ?? 0.0).toDouble(),
    );

    // Cálculo de cantidad de ubicaciones
    final cantidadUbicaciones = stockUbicacion
        .map((e) => e['ubicacion'] ?? '')
        .toSet()
        .length;
  ...
  // =========================================================================
  Map<String, dynamic> _calcularResumenAABC(
    List<Map<String, dynamic>> datosAABC,
  ) {
    double vcaTotal = 0.0;
  ...
  List<Map<String, dynamic>> _mapearCompras(List<Compra> compras) {
    ...
    return compras.map((c) {
      DateTime? fechaObj = (c.fechaHora is DateTime)
          ? c.fechaHora as DateTime
          : null;
  ...
  // ====================================================================
  List<Map<String, dynamic>> _mapearStockPorUbicacionParaVista(
    List<Map<String, dynamic>> stockUbicacion,
  ) {
    return stockUbicacion.map((item) {
      final ubicacion = _toText(
        item['ubicacion'] ??
            item['nombre_almacen'] ??
            item['almacen'] ??
            item['deposito'],
      ...
      final producto = _toText(
        item['nombre_producto'] ?? item['producto'] ?? item['nombre'],
      );

      final ref = _toText(
        item['sku'] ?? item['ref'] ?? item['codigo'] ?? item['id_producto'],
      );

      final cantidad = _toDouble(
        item['cantidad'] ?? item['stock'] ?? item['existencia'],
      );

      return {
        // Claves que el PDF debe poder leer
        'ubicacion': ubicacion,
        'ref': ref,
        'producto': producto,
        'cantidad': cantidad,

        // Claves para VistaReporteDetallado (layout genérico)
        'fecha': producto,
        'entidad': ubicacion,
        'total': cantidad,
        'timestamp': null,
  ...
  // ====================================================================
  List<Map<String, dynamic>> _mapearReporteAABCParaVista(
    List<Map<String, dynamic>> reporteAABC,
  ) {
  ...
  // Mapeo para Kardex
  // ====================================================================
  List<Map<String, dynamic>> _mapearKardexParaVista(
    List<Map<String, dynamic>> movimientos,
  ) {
    return movimientos.map((m) {
      DateTime? fechaObj = (m['fecha'] != null)
      ...
      double cantidadAbsoluta =
          double.tryParse(m['cantidad'].toString()) ?? 0.0;

      bool esSalida =
          tipo.toLowerCase().contains('salida') ||
          tipo.toLowerCase().contains('venta') ||
          tipo.toLowerCase().contains('consumo') ||
          tipo.toLowerCase().contains('out');

      double cantidadReal = esSalida
          ? (cantidadAbsoluta * -1)
          : cantidadAbsoluta;
  ...
  Future<void> _cargarYMostrarDetalleVenta(Venta venta) async {
    ...
    try {
      ...
      final detalles = await widget.ventaService.obtenerDetallesVenta(
        venta.idVenta!,
      );
      if (!mounted) return;
  ...
  Future<void> _cargarYMostrarDetalleCompra(Compra compra) async {
    ...
    try {
      ...
      final detalles = await widget.compraService.obtenerDetallesCompra(
        compra.idCompra!,
      );
      if (!mounted) return;
      Navigator.of(context).pop();
  ...
  void _mostrarDialogoDetalleCompra(
    Compra compra,
  ...
  Future<void> _imprimirStockPorUbicacion(
    List<Map<String, dynamic>> datos,
  ) async {
    try {
      final datosNormalizados = _mapearStockPorUbicacionParaVista(datos);
      ...
      await Printing.sharePdf(
        bytes: pdfBytes,
        filename: 'StockPorUbicacion.pdf',
      );
    } catch (e) {
      ...
      await _imprimirReporteGeneral(
        "Stock por Ubicación (Fallback)",
        datosMapeados,
        0,
      );
    }
  ...
  Future<void> _imprimirKardex(List<Map<String, dynamic>> datos) async {
    try {
      ...
      for (final item in datos) {
        final cantidad = (item['cantidad'] as num? ?? 0).toDouble();
        final tipo = (item['tipo_movimiento'] ?? '').toString().toLowerCase();
  ...
  Future<void> _imprimirReporteAABC(List<Map<String, dynamic>> datos) async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Error al generar PDF AABC: $e")));
    }
  }
  ...
  Future<void> _imprimirReporteKPI(_ReporteData data) async {
    print(
      '--- INICIO: _imprimirReporteKPI (Lógica Local y SIN FLECHAS UNICODE) ---',
    );
    try {
      ...
      final double margenBrutoCalculado = (totalVentasMesActual > 0)
          ? (totalVentasMesActual - totalComprasMesActual) /
                totalVentasMesActual
          : 0.0;

      // CÁLCULO DE TENDENCIAS
      final double tendenciaVentas = _calcularTendenciaPct(
        totalVentasMesActual,
        totalVentasMesAnteriorDummy,
      );
      final double tendenciaMargen = _calcularTendenciaPct(
        margenBrutoCalculado,
        margenBrutoAnteriorDummy,
      );
      final double tendenciaRotacion = _calcularTendenciaPct(
        rotacionInventarioActual,
        rotacionInventarioAnteriorDummy,
      );
      final double tendenciaStockCritico = _calcularTendenciaPct(
        stockCriticoActual,
        stockCriticoAnteriorDummy,
      );
      ...
      print(
        'KPI Data lista. Count: ${kpiData.length}. Enviando a PdfService...',
      );
      ...
      print(
        'PDF generado correctamente. Procediendo a compartir (Mismo flujo que otros reportes)...',
      );

      // 4. COMPARTIR/DESCARGAR
      await Printing.sharePdf(
        bytes: pdfBytes,
        filename: 'Reporte_KPI_Gerencial.pdf',
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Error al generar Reporte KPI. Revise la consola por detalles.",
          ),
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => VistaReporteDetallado(
          ...
          datosIniciales: _mapearStockPorUbicacionParaVista(
    ...
    return Scaffold(
      ...
      body: FutureBuilder<_ReporteData>(
        ...
        builder: (context, snapshot) {
          ...
          final double valorAABC = data.reporteAABC.fold(
            0.0,
            (sum, item) => sum + (item['vca'] as num? ?? 0.0).toDouble(),
          );
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildReportButton(
                    ...
                    subtitle:
                        "Incluye Ventas, Margen, Rotación de Inventario y Tendencias.",
                    ...
                    onTap: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text("Generando Reporte KPI..."),
                        ),
                      );
                  ...
                  _buildReportButton(
                    ...
                    subtitle:
                        "${data.reporteAABC.length} productos clasificados",
                    icon: Icons.sort_by_alpha,
                    color: Colors.purple.shade600,
                    onTap: () => _abrirReporteAABC(data),
                    onPrint: () => _imprimirReporteAABC(
                      _mapearReporteAABCParaVista(data.reporteAABC),
                    ), // Usamos el mapeo para obtener los datos de VCA/Clase
                  ),
                  const SizedBox(height: 16),
                  ...
                  _buildReportButton(
                    ...
                    subtitle:
                        "${data.stockPorUbicacion.length} registros en ${data.cantidadUbicaciones} ubicaciones",
                    icon: Icons.location_on,
                    color: Colors.orange.shade700,
                    onTap: () => _abrirReporteStockPorUbicacion(data),
                    onPrint: () =>
                        _imprimirStockPorUbicacion(data.stockPorUbicacion),
                  ),
                  const SizedBox(height: 16),
                ],
              ),
...
String _toText(dynamic v, {String fallback = '-'}) {
  if (v == null) return fallback;

  if (v is Map) {
    final inner = v['nombre'] ?? v['name'] ?? v['descripcion'] ?? v['label'];
    return _toText(inner, fallback: fallback);
  }
